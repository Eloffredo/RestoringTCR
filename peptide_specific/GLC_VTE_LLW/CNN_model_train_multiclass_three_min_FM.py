...
peptide1, peptide2, peptide3 = 'VTEHDTLLY','LLWNGPMAV','GLCTLVAML'
check1, check2, check3 = 1000,2500,2100

df1_1 = pd.read_csv(f'./Binders_{peptide1}.csv').drop_duplicates()
df2 = pd.read_csv(f'./Generated_binders_{peptide1}_FM_beta{beta:.2f}.csv').drop_duplicates()
df2 = df2[df2['CDR3b'].str.len()>7]
df2 = df2[~df2['CDR3b'].isin(df1_1['CDR3b'])].dropna()

data_pos_1 = pd.concat((df1_1.sample(170),df2.sample(P_samples - 170 ) )).drop_duplicates()
data_pos_1['labels'] = 0
df1_1['labels'] = 0

data_pos_2 = pd.read_csv(f'./Binders_{peptide2}.csv').drop_duplicates()
data_pos_2['labels'] = 1

df1_3 = pd.read_csv(f'./Binders_{peptide3}.csv').drop_duplicates()
df2 = pd.read_csv(f'./Generated_binders_{peptide3}_FM_beta{beta:.2f}.csv').drop_duplicates()
df2 = df2[df2['CDR3b'].str.len()>7]
df2 = df2[~df2['CDR3b'].isin(df1_3['CDR3b'])].dropna()

data_pos_3 = pd.concat((df1_3.sample(600),df2.sample(P_samples - 600) )).drop_duplicates()
data_pos_3['labels'] = 2
df1_3['labels'] = 2
...
f ='./AUCACC_performance_multiclass_epochs_{}_batchsize_{}_three_min_FM.csv'.format(EPOCHS,batch_size)
if os.path.isfile(f):
